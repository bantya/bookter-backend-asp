...
namespace DAL
{
    public class BookDBManager
    {
        ...
        public static List<Books> Getbooks()
        {
            ...
            try
            {
                ...
                foreach (DataRow row in rows)
                {
                    ...
                    theproducts.image = row["image1"].ToString();
        ...
        public static List<Books> getAllBooks(int custid)
        {
            List<Books> books = new List<Books>();

            IDbConnection conn = new MySqlConnection();
            conn.ConnectionString = connString;
            string query = "SELECT b.booksID,b.bookname,b.image1 FROM orderr o JOIN orders os ON os.order_id = o.order_id JOIN books b ON b.booksID = o.book_id JOIN customers c ON c.customerID = os.user_id WHERE c.customerID = @customer";
            IDbCommand cmd = new MySqlCommand();
            cmd.CommandText = query;
            cmd.Connection = conn;
            cmd.Parameters.Add(new MySqlParameter("@customer", custid));
        ...
        public static int Insertbook(Books newbook)
        {
            ...
            try
            {
                using (MySqlConnection con = new MySqlConnection(connString))
                {
                    ...
                    string query = "INSERT INTO books (booksID,bookname,bookauthor,bookpage,booklang,bookpublisher,publishdate,bookdimension,paperprice,hardprice,ebookprice,bookdisc,aboutauthor,rating,image1,image2,image3,status) VALUES" +
                                                        "(default,@bname,@aname,@bpages,@blang,@bpub,@bdate,@bdimen,@bpp,@bhp,@ebp,@bdisc,@aathor,@brat,@bimag,@bimag2,@bimag3,@status); SELECT LAST_INSERT_ID()";
                  //  @bid,@bname,@aname,@bpages,@blang,@bpub,@bdate,@bdimen,@bpp,@bhp,@ebp,@bdisc,@aathor,@brat,@bimag,@bimag2,@bimag3
                    MySqlCommand cmd = new MySqlCommand(query, con);
                    //cmd.Parameters.Add(new MySqlParameter("@bid", newbook.booksID));
                    ...
                    cmd.Parameters.Add(new MySqlParameter("@bimag", "NULL"));
                    cmd.Parameters.Add(new MySqlParameter("@bimag2", "NULL"));
                    cmd.Parameters.Add(new MySqlParameter("@bimag3", "NULL"));

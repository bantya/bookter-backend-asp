        ...
        public static List<Books> GetWishList(int cid)
        {
            ...
            try
            {
                using (MySqlConnection con = new MySqlConnection(connString))
                {
                    ...
                    string query = "select booksID, bookname, bookauthor, publishdate, bookpublisher, paperprice, rating, image1 from books where booksID in (SELECT book_id FROM wishlist where customer_id = @cid)";
                    ...
                    using (MySqlDataReader reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            theproducts.image = reader["image1"].ToString();
